...
| 2026-06-18 | Per-vendor `coverPosition` (object-position) + baked Cosmos crop | MOTO logo lives at the photo's bottom edge → `object-bottom` so it survives the cover crop. Cosmos is a wide 3:2 shot in a 5:4 tile (crops left/right, not top/bottom), so a vertical CSS nudge does nothing — re-cropped the source to 5:4 (zoom + headroom) instead. See `/tmp` crop helper |
| 2026-06-18 | Cosmos modal anchored `object-bottom`; Lobster Lab re-fit to show all 4 rolls | Cosmos baked-5:4 cover crops top/bottom in the 16:9 modal — anchored bottom so the board + bun bases survive (grid tile unaffected, exact fit). Lobster Lab 16:9 shot lost the outer rolls in the 5:4 tile → kept full width and extended the table top/bottom with a blurred color-matched fill (AI outpaint deferred: Higgsfield/Pixa at 0 credits) |
| 2026-06-18 | Handel's Ice Cream → square shot; added `coverWide` for the modal | Square cone fits the 5:4 grid tile. In the 16:9 modal the square cropped to a zoomed slice, so added an optional per-vendor `coverWide` (16:9, sides extended with a blurred color-matched fill) used by the modal — full cone visible, no zoom. Falls back to `cover` for all other vendors |
